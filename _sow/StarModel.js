...
module.exports = function (options) {
  ...
  //  this.soletHost='solet.yuanjin.net';

  this.setModel(options)

  /*
   * 设置系统后台的地址，SOLET为后台代码根地址，SOPIC为后台图片文件根地址。
   * 搜索顺序：1. 当前应用目录下的solet目录，2. yuanjin.net下的solet目录（如果前者不存在，或当前是手机混合应用）。
   * 注意 1.为了解决session跨域问题，不能使用固定的后台地址yuanjin.net，而要使用linkd.exe建立的硬链接solet
   *   2.如果浏览地址是 .../index.php 等文件结尾，window.location.pathname也包含这些，因此要替换掉这些。
   */
  if (sow.Tool.deviceInfo().appmode === 'web') {
    // 是浏览器访问。测试protocol + host + pathname是否有效. 对cordova或html5+的打包应用，href=file:///android_assets/www/... 或 file:///var/mobile/Containers/Data/Application/... (host=空), 不要交给urlExists。
    ...
    if (/^localhost|127.0.0.1|192\.168\.\d+\.\d+$/.test(window.location.hostname)) {
      ...
      //      SOLET= window.location.protocol + '//' + window.location.hostname + (/^https:$/.test(window.location.protocol)?':8765/':':8765/');
      SOLET = 'http://' + window.location.hostname + ':8765/'
    } else {
      // if internet
      //      SOLET= window.location.protocol + '//' + 'solet.yuanjin.net' + (/^https:$/.test(window.location.protocol)?':8765/':':8765/');
  ...
  this.simplize = function (obj) {
    // 去掉 _data 和 函数等不需要传给后台的东西，节省流量。
    if (obj && typeof obj === 'object' && obj.hasOwnProperty('_data')) {
      var newObj = {}
      for (var p in obj) {
        if (obj.hasOwnProperty(p) && typeof obj[p] !== 'function' && p !== '_data')
          // maybe also delete '_class'?
  ...
  this.normalize = function (postdata) {
    // 定义在 starModel 里，就能使用 starId, webInfo 等信息了。
    postdata = postdata || {}
  ...
  $.post(
    SOLET + 'Session_addOne',
    self.normalize({ session: { webInfo: self.webInfo, star: { id: self.starId, url: self.starUrl, version: self.starVersion, name: self.starName } } }),
    function () {},
    'json'
  ) // 不知为何，navigator不能直接作为参数；而JSON.stringify(navigator)得到的是无关紧要的数据，不包含appName的信息。因此要手工制作 navigator 对象。
  ...
  if (window.plus) {
    ...
    document.addEventListener('plusready', webappReady, false)
  } // 监听 HTML5plus(H5+)。
  function webappReady () {
    if (window.plus && window.plus.device) {
      // H5+应用。有时忘了在HBuilder里定制manifest.json，没有添加device权限，则device=undefined, 存取plus.device.xxx 导致静悄悄出错，因此要确认plus.device存在。
      ...
      self.appInfo = {
        ...
        uuid: plus.device.uuid, // 如果直接赋值为plus.device，会在应用启动后试图打开电话拨号，因为plus.device还包含几个方法。
        screen: plus.screen,
    ...
    window.plus.key.addEventListener(
      ...
      function () {
        ...
        plus.nativeUI.confirm(
          '真的要退出应用吗？', // 按 Home 键可切换到其他应用，不需要退出哦。
          function (evt) {
            if (evt.index === 1) window.plus.runtime.quit()
          }, // confirmCallback
  ...
  this.where = ko.computed(function () {
    if (self.targetPerson() == sow.Const.PERSON_ALL)
      // 在广场
      return 'SPACE_ALL'
    else if (self.targetPerson() != self.onlinePerson() && self.targetPerson() != sow.Const.PERSON_UNKNOWN)
      // 在别家
      return 'SPACE_HOST'
    else if (self.targetPerson() == self.onlinePerson() && self.onlinePerson() != sow.Const.PERSON_UNKNOWN)
      // 在自家
      return 'SPACE_HOME'
    else return 'SPACE_SHARE' // target == anonym
  })
