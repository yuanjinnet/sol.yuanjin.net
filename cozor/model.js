 ...
 ** Web: http://sol.yuanjin.net/cozor/
...
window.starModel = new sow.StarModel({ starVersion: '2017081800', starCode: 'cozor', starName: '创作社' })
  ...
  this.showProject = function (project, evt, stage, scene, callback) {
    ...
    if (!starModel.isOnline()) {
      ...
      if (project._data.owner) {
        ...
        urProject.updateCare(project) // 不在线时不存在关注关系，但如果是用户退出登录，就要取消原本登录时的状态！
...
*/
  ...
  this.joinPerson = function (person, personIndex) {
    var self = this
    var observableCircle = self.roleNow
    var observableList = self.roleList
    var listIndex = self.roleIndex
...
window.myHome = new (function (options) {
  ...
  accountModel.addListener('LOGOUT_SUCCESS', function () {
    ...
    self.proset([])
    self.noticeList([])
    //    urHost.logList(self.logList()); urHost.logNow(self.logNow()); urHost.sceneLog(self.sceneLog()); self.logList([]);
